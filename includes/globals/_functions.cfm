...
<cffunction name="v4renderBreadCrumb" access="public" returntype="boolean" hint="Renders bread crumb for nav.nav item" output="yes">
	...
	<cftry>
		<cfif (nav.open or nav.selected) and (right(trim(nav.getPath()),9) NEQ 'index.cfm' OR findNoCase('online',nav.getPath()) or right(trim(nav.getPath())) NEQ CGI.script_name)>
			#seperator#
